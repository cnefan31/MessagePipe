# MyMessagePipe AOT Support — 改造总结

## 一、背景与目标

MyMessagePipe (基于 MessagePipe) 是一个高性能的 .NET 内存/分布式消息管道库，广泛用于 Pub/Sub、CQRS Mediator 模式、EventAggregator、IPC-RPC 等场景。
...
**发布包名：** 为避免与原始 MessagePipe 包冲突，所有 AOT 包使用 `MyMessagePipe.*.Aot` 命名前缀。
...
1. **`AddMessagePipe()` 仍可用但非 AOT 安全** — 原有的基于反射的注册方法未修改，在 JIT 模式下正常工作；AOT 场景请使用 `AddMessagePipeAot()`（由 MyMessagePipe.SourceGenerator.Aot 生成）
